...
department int references department(department_id) ON DELETE cascade on update cascade,
hire_date date,
position varchar(20),
manager int,
location int references location(location_id) ON DELETE cascade on update cascade
);
...
--Task Two
select department , ROUND(AVG(monthly_salary),0) 
from employee
group by department;
